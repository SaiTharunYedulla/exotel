...
async function pcmToWavBuffer(pcmBuffer) {
  return new Promise((resolve, reject) => {
    const writer = new wav.Writer({
      sampleRate: 8000, // match Exotel PCM rate
      channels: 1,
      bitDepth: 16,
    });
    const chunks = [];
...
wss.on("connection", (socket) => {
  ...
  // Process chunks immediately every X seconds
  ...
  // Regularly process audio chunks
  const interval = setInterval(() => processAudio(), PROCESS_INTERVAL);
